...
class Runner {
    ...
    if (!IS_IOS) {
      ...
      for (const sound in Runner.sounds) {
        ...
        /*this.audioContext.decodeAudioData(buffer, function(index, audioData) {
          ...
          }.bind(this, sound));*/'yadayadayadayadaya'
      }
    }
  }
  ...
  init() {
    ...
    /*document.querySelector('.' + Runner.classes.ICON).style.visibility =
        'hidden';*/ 'no need for this crap'

    this.adjustDimensions();
    this.setSpeed();

    const ariaLabel = 'skibidi-united-thingy';
    this.containerEl = document.createElement('div');
    ...
    announcePhrase("Chrome's signature dino run game.");
  ...
  playIntro() {
    if (!this.activated && !this.crashed) {
      ...
      const keyframes = '@-webkit-keyframes intro { ' +
          ...
          '}';
      document.getElementById("cssimportant").innerHTML = keyframes;
  ...
  startGame() {
    ...
    announcePhrase("Started!");

    if (Runner.audioCues) {
      this.containerEl.setAttribute('title', 'dino run');
    }
      ...
      if (!this.crashed && !this.paused) {
        ...
        if (Runner.keycodes.JUMP[e.keyCode] ||
          ...
          if (!this.playing) {
            ...
            this.setPlayStatus(true);
            this.update();
            this.highestScore = parseInt(localStorage.getItem("hiscore"))
          }
          // Start jump.
  ...
  saveHighScore(distanceRan, opt_resetScore) {
    ...
    if (this.syncHighestScore) {
      if (opt_resetScore) {
        this.distanceMeter.setHighScore(0);
...
function decodeBase64ToArrayBuffer(base64String) {
  ...
  return bytes.buffer;*/ 'as we are running the sound on a url, this is easy, just use the playsound method'
}
